<template>
  <div>
    <div
      class="mainMovieContainer"
      :class="[
        $q.screen.width < 700 ? 'moviesContainerMobile' : 'movies-container',
      ]"
    >
      <div
        v-for="movie in limitedMovies"
        :key="movie.id"
        :class="[$q.screen.width < 700 ? 'movieMobile' : 'movie']"
      >
    ...
    <div>
      ...
      <p class="text-grey" style="font-size: 20px" align="center">
  ...
  setup() {
    ...
    const fetchPopularMovies = async () => {
      try {
        ...
        console.log(
          `Время загрузки всех фотографий: ${endTime - startTime} мс`
        );
